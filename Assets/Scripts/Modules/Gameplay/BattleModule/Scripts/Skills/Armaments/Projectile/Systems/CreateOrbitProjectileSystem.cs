    ...
    public partial struct CreateOrbitProjectileSystem : ISystem
    {
        public void OnUpdate(ref SystemState state) {
            ...
            foreach (var request in SystemAPI.Query<RefRO<CreateOrbitProjectile>>()) {
                var ability = request.ValueRO.Skill.Value;
                ...
                ecb.AddComponent(projectile, new Effects{ Ability = request.ValueRO.Skill });
                ecb.AddComponent(projectile, new Statuses{ Ability = request.ValueRO.Skill });
                ecb.AddComponent(projectile, new Stats{ Ability = request.ValueRO.Skill });
