...
public partial class TranslucentImage : Image, IMeshModifier
{
    ...
    void AutoAcquireSource()
    {
        ...
        source                = source ? source : FindSourceInScene();
        sourceAcquiredOnStart = source != null;
    }

    static TranslucentImageSource FindSourceInScene()
    {
#if UNITY_2023_1_OR_NEWER
        var sources = UnityEngine.Object.FindObjectsByType<TranslucentImageSource>(FindObjectsInactive.Exclude);
#else
        var sources = FindObjectsOfType<TranslucentImageSource>();
#endif
        TranslucentImageSource bestSource = null;
        ...
        foreach (var candidate in sources)
        {
            ...
            var camera = candidate.GetComponent<Camera>();
            var depth  = camera ? camera.depth : float.NegativeInfinity;
            if (bestSource != null && depth <= bestDepth)
                continue;
